...
> 프론트엔드부터 백엔드, CI/CD까지 직접 부딪혀 본 풀스택 지향 개발자입니다.
> 반도체 제조 현장에서 쌓은 데이터 분석 습관을 소프트웨어 설계로 이어가고 있으며,
> **풀스택 개발**을 목표로 성장하고 있습니다.
...
### 🩺 GluCoach — 당뇨 환자 혈당·식단 관리 앱
> CGM 데이터 기반 실시간 혈당 측정 · 혈당 예측 AI · 음식 인식 · RAG · AI Agent
- **Role**: Backend 리더 / Git 버전 관리
- **Stack**: `Spring Boot` `Java 21` `PostgreSQL` `Redis`
- 

### 💼 GGulJob — AI 채용 로드맵 플랫폼
> 사용자 맞춤 채용 로드맵 + Claude API 기반 챗봇
- **Role**: Frontend 리더
- **Stack**: `React` `TypeScript` `Zustand` `React Query`
- 렌더링 최적화로 응답 속도 약 5초 → 1초 미만 단축

### 🎙️ HearO — 실시간 음성 상담 시스템
> WebRTC 기반 실시간 음성 통신 서비스
- **Role**: Frontend 리더
- **Stack**: `WebRTC` `LiveKit` `WebSocket` `React`
...
![Bootstrap](https://img.shields.io/badge/Bootstrap-%237952B3?style=plastic&logo=Bootstrap&logoColor=white)
...
![Jenkins](https://img.shields.io/badge/Jenkins-%23D24939?style=plastic&logo=Jenkins&logoColor=%23ffffff)
![Nginx](https://img.shields.io/badge/Nginx-%23009639?style=plastic&logo=Nginx&logoColor=%23ffffff)
![AWS](https://img.shields.io/badge/AWS-%23232F3E?style=plastic&logo=amazonwebservices&logoColor=%23ffffff)
